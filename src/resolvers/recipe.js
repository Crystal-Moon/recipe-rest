  ...
  Query: {
    ...
    getOneRecipe: combineResolvers( isAuthenticated,
      async(_, { id }, { who })=> {
        ...
        if(!recipe) throw new ApolloError(E400.NOT_FOUND[who.lang]);
  ...
  Mutation: {
		createRecipe: combineResolvers( isAuthenticated, 
		  async(_, newRecipe, { who })=> {
		  	...
		  	return conn().getRepository(Category)
                .findOne({ name: newRecipe.category.name.toLowerCase(), author: who })
		  	    .then(async category=> {
    ...
    updateRecipe: combineResolvers( isAuthenticated,
    	async(_, update, { who })=> {
    	  ...
    	  if(recipe.author.id != who.id) throw new ForbiddenError(E400.NOT_PERMISSION[who.lang]);

    	  let bad = await verifyFields('recipe', update, who.lang);
    	  if(bad) throw new UserInputError(bad);

    	  return conn().getRepository(Category)
              .findOne({ name: update.category.name.toLowerCase(), author: who })
		  	  .then(async category=> {
